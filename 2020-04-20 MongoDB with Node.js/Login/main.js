...
var app = http.createServer(function(request, response){
	...
	if(pathname==='/'){//홈
		...
		request.on('end', function(){
			...
			var mypage =`
				...
				<a href='remove?id=${postid}'>회원탈퇴</a>
				`;
	...
	}else if(pathname==='/remove'){ // 회원탈퇴
		var id = queryData.id;
		Member.remove({"id":id}, function(error,output){
			if(error){
				console.log(error);
			}else{
				console.log("삭제성공");
